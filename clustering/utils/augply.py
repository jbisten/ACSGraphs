...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--infile', type=str, help='input .ply file for processing')
   ...
   # Interpolate on vertices
    ...
    sub_ids = [find_subject_descriptor(infile) for i in range(len(end_indices))]
    side_ids = np.zeros(len(sub_ids)) if 'lh' in infile.parts[-1] else np.ones(len(sub_ids))
